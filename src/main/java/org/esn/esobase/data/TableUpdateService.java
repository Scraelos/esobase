...
public class TableUpdateService {
    ...
    public void updateAbilityDescriptions() {
        Query q0 = em.createNativeQuery("with newid as (select g.id,e.aid,e.bid,e.cid from gspreadsheetsabilitydescription g join esorawstring e on lower(regexp_replace(e.texten,'\\n','$','g'))=lower(g.texten) and e.aid in(132143172,121548292) order by e.aid asc,e.cid asc,e.bid asc) update gspreadsheetsabilitydescription set deprecated=FALSE,aid=newid.aid,bid=newid.bid,cid=newid.cid,weight=newid.cid from newid where gspreadsheetsabilitydescription.id=newid.id;");
        q0.executeUpdate();
    ...
    public void updateNpcNames() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            q8.executeUpdate();
            Query qDelete = em.createNativeQuery("delete from gspreadsheetsnpcname where id in (select max(g2.id) as gid2 from gspreadsheetsnpcname g1 join gspreadsheetsnpcname g2 on lower(g1.texten)=lower(g2.texten) and ((g1.sex=g2.sex) or (g2.sex is null and g1.sex is null)) and g1.id!=g2.id group by lower(g1.texten),g1.sex);");
            qDelete.executeUpdate();
        }
